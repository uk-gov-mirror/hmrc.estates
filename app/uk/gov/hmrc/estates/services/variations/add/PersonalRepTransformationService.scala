...
class PersonalRepTransformationService @Inject()(transformationService: VariationsTransformationService)
  ...
  def addAddBusinessPersonalRepTransformer(utr: String, internalId: String, newPersonalRep: EstatePerRepOrgType): Future[Success.type] = {
    transformationService
      .addNewTransform(utr, internalId, AddBusinessPersonalRepTransform(newPersonalRep))
      .map(_ => Success)
  }
