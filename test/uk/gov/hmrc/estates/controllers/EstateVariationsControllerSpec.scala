...
import uk.gov.hmrc.estates.utils.{Headers, JsonRequests}
...
class EstateVariationsControllerSpec extends BaseSpec with BeforeAndAfterEach {
  ...
  private def estateVariationsController = {
    new EstateVariationsController(new FakeIdentifierAction(bodyParsers, Organisation), mockDesService, mockAuditService, validationService, mockConfig, responseHandler)
  }
